...
export class FeatureDetectionController {
  ...
  @EventPattern({ cmd: 'hough_transform_image' })
  async handleHoughTransform(imagePath: string) {
    console.log('Received image for hough transform');
    return await this.featureDetectionService.houghTransform(imagePath);
  }
